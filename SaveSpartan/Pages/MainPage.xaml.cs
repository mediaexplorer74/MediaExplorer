    ...
    public sealed partial class MainPage : Page
    {
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (e.Parameter is IActivatedEventArgs args && NavigatedPageType != typeof(SettingsPage))
            {
                ...
                if (args.Kind == ActivationKind.Protocol)
                {
                    ...
                    if (protocolArgs.Uri.AbsoluteUri == "MediaExplorer:" && protocolArgs.Data != null)
                    {
                        ...
                        else if (protocolArgs.Data.ContainsKey("filePath"))
                        {
                            ...
                            catch (Exception)
                            {
                                ContentDialog unauthorizedAccessDialog = new ContentDialog()
                                {
                                    ...
                                    Content = "\r\nMediaExplorer can't access the file at " + "\"" + protocolArgs.Data["filePath"].ToString() + "\".",
                    ...
                    else if (protocolArgs.Uri.AbsoluteUri.StartsWith("MediaExplorer:Go"))
                    {
        ...
        private void Navigate(string address, bool newTab = false)
        {
            ...
            if (!protocolRegex.IsMatch(address))
            {
                ...
                {
                    Regex schemeRegex = new Regex(@"^(javascript|data|chrome|mailto|tel|sms|callto|mms|MediaExplorer|textie|ms-settings|windowsdefender|magnet):?(\S)+$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        private async void PinSiteToStartMenu()
        {
            ...
            {
                new UCNotification("Error", "Something gone wrong. MediaExplorer can't pin the tile.").ShowNotification();
            }
        }
