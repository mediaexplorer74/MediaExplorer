...
### Status
- **Data extraction**: Working from 589KB JS chunk (brace-counting) → `__graphData` (755n + 1647l)
- **Graph rendering**: Circular layout via SVG→XAML pipeline, 145KB XML
- **Timeline rendering**: Compact greedy-packed rows (230 stories → 140 rows, 490px), 31KB XML
- **Route navigation**: Disabled (was causing double render cycle)
- **Automation**: Full loop — build → deploy → 120s wait → kill → read logs → repeat

### Key files
`JavaScriptEngine.cs` lines 3515 (data), 3560 (graph SVG), 3636 (timeline SVG), 4092 (route)
