        ...
        do
        {
            ...
            switch (state.Code[i])
            {
                ...
                case '=':
                {
                    ...
                    // The 'first' operand is the parameter(s), and we see '=' followed by '>'
                    if (state.Code[i + 1] == '>')
                    {
                        // This is an arrow function - consume it entirely and return directly
                        i += 2; // skip =>
                        while (state.Code.Length > i && Tools.IsWhiteSpace(state.Code[i]) && !Tools.IsLineTerminator(state.Code[i]))
                            i++;

                        Expression arrowFunc;
                        if (state.Code[i] == '{')
                        {
                            // Block body
                            ...
                            using (state.WithCodeContext())
                            {
                                ...
                                try
                                {
                                    ...
                                    var bodyBlock = CodeBlock.Parse(state, ref i) as CodeBlock;

                                    var parameters = new List<ParameterDescriptor>();
                                    if (first is VariableReference vr)
                                    {
                                        var pd = new ParameterDescriptor(vr.Name, false, state.LexicalScopeLevel);
                                        parameters.Add(pd);
                                    }

                                    arrowFunc = new FunctionDefinition()
                                    {
                        ...
                        {
                            // Expression body
                            var bodyExpr = ExpressionTree.Parse(state, ref i, processComma: false);
                        ...
                        // Return the arrow function directly, don't continue the loop
                        return arrowFunc;
                    }
                    if (state.Code[i + 1] == '=')
                    {
