...
public sealed class FunctionDefinition : EntityDefinition
{
        ...
        try
        {
            ...
            if (code[position] != '{')
            {
                if (kind == FunctionKind.Arrow || kind == FunctionKind.AsyncArrow)
                {
                    ...
                    // Set InAsync for expression body async arrow functions
                    var oldContext = state.CodeContext;
                    if (kind == FunctionKind.AsyncArrow)
            ...
            else
            {
                ...
                using (state.WithCodeContext())
                {
                    if (kind == FunctionKind.Generator || kind == FunctionKind.MethodGenerator || kind == FunctionKind.AnonymousGenerator || kind == FunctionKind.AsyncGenerator)
                        state.CodeContext |= CodeContext.InGenerator;
                    if (kind == FunctionKind.AsyncFunction || kind == FunctionKind.AsyncMethod || kind == FunctionKind.AsyncAnonymousFunction || kind == FunctionKind.AsyncArrow || kind == FunctionKind.AsyncGenerator)
                        state.CodeContext |= CodeContext.InAsync;
            ...
            // Allow function call immediately after declaration: function f(){}()
            // but NOT if the first argument starts with '(' (which would be IIFE)
            // or '[' (computed property), or '`' (tagged template)
            {
                var tindex = position;
                ...
                {
                    // Check if this looks like an IIFE call (argument starts with function/class/async)
                    // In that case, don't parse as call - let it be a separate statement
                    var checkPos = position + 1;
                    while (checkPos < code.Length && Tools.IsWhiteSpace(code[checkPos]))
                        checkPos++;
                    
                    bool isIifeStyle = false;
                    if (checkPos < code.Length)
                    {
                        if (code[checkPos] == '(' || code[checkPos] == '[' || code[checkPos] == '`')
                            isIifeStyle = true;
                        if (Parser.Validate(code, "function", checkPos) || Parser.Validate(code, "class", checkPos) || Parser.Validate(code, "async", checkPos))
                            isIifeStyle = true;
                    }
    ...
    {
        if (_kind == FunctionKind.Generator || _kind == FunctionKind.MethodGenerator || _kind == FunctionKind.AnonymousGenerator || _kind == FunctionKind.AsyncGenerator)
    ...
    public override bool Build(ref CodeNode _this, int expressionDepth, int scopeLevel, Dictionary<string, VariableDescriptor> variables, CodeContext codeContext, InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        if (!string.IsNullOrEmpty(_name) && _kind is FunctionKind.Function or FunctionKind.Generator or FunctionKind.AsyncFunction or FunctionKind.AsyncGenerator)
        {
        ...
        if (!string.IsNullOrEmpty(_name) && (_kind == FunctionKind.Function || _kind == FunctionKind.Generator || _kind == FunctionKind.AsyncFunction || _kind == FunctionKind.AsyncGenerator))
