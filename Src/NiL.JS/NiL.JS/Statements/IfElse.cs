...
public sealed class IfElse : CodeNode
{
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        if (body is FunctionDefinition)
        {
            ...
            body = new CodeBlock([body]); // дл€ того, чтобы не дублировать код по декларации функции, 
            // она оборачиваетс€ в блок, который сделает самовыпил на втором этапе, но перед этим корректно объ€вит функцию.
        }
        ...
        if (Parser.Validate(state.Code, "else", ref i))
        {
            ...
            if (elseBody is FunctionDefinition)
            {
                ...
                elseBody = new CodeBlock([elseBody]); // дл€ того, чтобы не дублировать код по декларации функции, 
                // она оборачиваетс€ в блок, который сделает самовыпил на втором этапе, но перед этим корректно объ€вит функцию.
            }
        }
        else
