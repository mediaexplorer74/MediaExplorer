...
public sealed class VariableDefinition : CodeNode
{
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index, bool forForLoop)
    {
        ...
        // After var/let/const, expect whitespace or destructuring chars ({, [)
        // Minified code may have const{x} without space
        if (!Tools.IsWhiteSpace(state.Code[position]) && state.Code[position] != '{' && state.Code[position] != '[')
            return null;
