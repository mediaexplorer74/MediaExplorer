        ...
        using (state.WithCodeContext())
        {
            ...
            try
            {
                ...
                while ((sroot && position < state.Code.Length) || (!sroot && state.Code[position] != '}'))
                {
                    ...
                    // ASI: after function/class declaration, insert implicit semicolon
                    // if next token could start an expression
                    if (t is EntityDefinition && position < state.Code.Length)
                    {
                        var next = state.Code[position];
