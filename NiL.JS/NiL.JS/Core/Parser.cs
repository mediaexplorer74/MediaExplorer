...
public static class Parser
{
    ...
    [
        ...
        new List<Rule> // Общий
        {
            ...
            new Rule(ValidateVarLetConst, VariableDefinition.Parse),
    ...
    private static bool ValidateVarLetConst(string code, int index)
    {
        if (Parser.Validate(code, "var", ref index) || Parser.Validate(code, "let", ref index) || Parser.Validate(code, "const", ref index))
        {
            if (index >= code.Length) return false;
            var c = code[index];
            return Tools.IsWhiteSpace(c) || c == '{' || c == '[' || Parser.ValidateName(code, index);
        }
        return false;
    }
